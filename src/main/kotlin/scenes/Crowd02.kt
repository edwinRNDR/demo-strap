...
class Crowd02 {
    ...
    var lastUpdate = (0..1000).map { -10.0 }.toMutableList()
    var durations = (0..1000).map { (Math.random() * 500 + 500)/1000.0 }.toMutableList()

    fun update(time:Double) {
        for (i in 0 until lastUpdate.size) {
            if (time - lastUpdate[i] > durations[i] || lastUpdate[i] > time) {
                positions[i] =
    ...
    fun drawShadow(drawer: Drawer, time: Double, renderStyle: RenderStyle = RenderStyle()) {
        drawer.isolated {
            ...
            for (person in persons) {
                val m = (time - lastUpdate[index]).coerceIn(0.0, 1.0)
                drawer.isolated {
